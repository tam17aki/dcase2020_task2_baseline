...
def file_to_vector_array(file_name,
    ...
    for t in range(frames):
        vector_array[:, n_mels * t: n_mels * (t + 1)] = \
...
def file_to_vector_array_MFCC(file_name,
                              n_mels=64,
                              n_mfcc=40,
                              mfcc_order=2,
                              frames=5,
    ...
    mfcc = librosa.feature.mfcc(y=y,
                                ...
                                power=power)
    if mfcc_order == 2:
        delta = librosa.feature.delta(mfcc, order=1, width=13)
        feat = numpy.concatenate([mfcc, delta])
    elif mfcc_order == 3:
        delta = librosa.feature.delta(mfcc, order=1, width=13)
        accel = librosa.feature.delta(mfcc, order=2, width=13)
        feat = numpy.concatenate([mfcc, delta, accel])
    else:
        feat = mfcc
    ...
    # 04 calculate total vector size
    vector_array_size = len(feat[0, :]) - frames + 1
    ...
    vector_array = numpy.zeros((vector_array_size, dims))
    for t in range(frames):
        vector_array[:, n_feat * t: n_feat * (t + 1)] = \
            feat[:, t: t + vector_array_size].T
